...
description: Use when refreshing, cleaning, or removing the Crema Arena design system bundle at `.design-system/crivelo-design-system/`. Takes a handoff URL from claude.ai/design, downloads it, replaces the existing bundle, and strips files that duplicate the repo (project/app, app_spec.txt, tailwind.config.ts, uploads, chats).
user-invocable: true
---
...
```
.design-system/
└── crivelo-design-system/      # the bundle (replaceable reference data)
...
1. Confirm: "Replace bundle at `.design-system/crivelo-design-system/`? Existing one will be backed up as `.bak-<timestamp>/`."
...
4. Extract:
   ...
   ```
5. Sanity check: `ls /tmp/design-new/crivelo-design-system/` must show `README.md` and `project/`. If not, abort and surface what was inside.
6. Backup: `mv .design-system/crivelo-design-system .design-system/crivelo-design-system.bak-$(date +%s)`.
7. Install: `mv /tmp/design-new/crivelo-design-system .design-system/`.
8. Run **Clean** (below).
9. Diff vs the backup so the user sees what changed:
   ```bash
   diff -rq .design-system/crivelo-design-system.bak-<ts>/ \
            .design-system/crivelo-design-system/ | head -40
   ```
...
```bash
rm -rf .design-system/crivelo-design-system/project/app
rm -rf .design-system/crivelo-design-system/project/uploads
rm -rf .design-system/crivelo-design-system/chats
rm -f  .design-system/crivelo-design-system/project/app_spec.txt
rm -f  .design-system/crivelo-design-system/project/tailwind.config.ts
rm -f  .design-system/crivelo-design-system/project/SKILL.md
```
...
`rm -rf .design-system/crivelo-design-system/` — confirm first. The `crema-arena-design` skill keeps loading (its SKILL.md is in `.claude/skills/`), but its references to `preview/`, `ui_kits/`, and the brand `README.md` will dangle until a new bundle is fetched.
